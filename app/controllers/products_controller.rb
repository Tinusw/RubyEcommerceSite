class ProductsController < ApplicationController
  ...
  def index
    if params[:q]
      ...
      @products = Product.where("lower(name) ILIKE lower(?)", "%#{search_term.downcase}%").paginate(:page => params[:page], :per_page => 12)
  ...
  private
    ...
    def product_params
      params.require(:product).permit(:name, :desc, :image_url, :style, :price)
    end
end
